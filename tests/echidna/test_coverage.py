...
to_test = ["ExploreMe.sol", "Primality.sol"]
to_test = [CONTRACTS_DIR / contract_file for contract_file in to_test]

# Test coverage on every contract
@pytest.mark.parametrize("contract", to_test)
def test_coverage(contract: str):
    ...
    # Run hybrid echidna
    cmdline_args = f"{contract}  --test-mode assertion --corpus-dir {test_dir} --seq-len 1 --seed 123456 --max-iters 10 --test-limit 10000 --debug".split()
